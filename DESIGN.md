...
`scripts/fetch-tags.js`). These use a compact indexed format where tag labels
are stored once in a sorted array (`t`) and each ID maps to integer indices
into that array (`d`). This reduces the bundled size to ~12 MB total
